...
'''
path = raw_data_path + "industry/CSI300 concepts.xlsx"
T = pd.read_excel(path, index_col=0)
T_ts_code = T['ts_code']
T['ts_code'] = T['ts_code'].apply(lambda x: x[:-3])
T = T.values
path = raw_data_path + "industry/CSI500 industry.xlsx"
I = pd.read_excel(path, index_col=0)
I_ts_code = I['ts_code']
I['ts_code'] = I['ts_code'].apply(lambda x: x[:-3])
I = I.values
path = raw_data_path + "industry/CSI300 shareHolders.xlsx"
S = pd.read_excel(path, index_col=0)
S = S[['ts_code', 'hold_ratio', 'holder_code']]
S_ts_code = S['ts_code']
S['ts_code'] = S['ts_code'].apply(lambda x: x[:-3])
def check(x):
    if x == 'no':
        return '0'
    else:
        return x[:-3]
S['holder_code'] = S['holder_code'].apply(lambda x: check(x))
S = S.values
...
Epochs = 1000
Batch_size = 32
History = model.fit(x_train, y_train, batch_size=Batch_size, epochs=Epochs, callbacks=[model_checkpoint],
